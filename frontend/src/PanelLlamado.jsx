...
export default function PanelLlamado() {
  ...
  const [ventanilla, setVentanilla] = useState("A1");
  ...
  const repetirLlamado = async () => {
    if (loading || !actual || !actual.id_ticket) return;
    try {
      await api.post(`/tickets/${actual.id_ticket}/repetir`, { ventanilla });
      await cargarActual();
    } catch (e) {
      alert(e?.response?.data?.mensaje || "No se pudo repetir el llamado");
    }
  };
  ...
  const finalizar = async () => {
    ...
    try {
      ...
      alert(e?.response?.data?.mensaje || "No se pudo finalizar");
    } finally {
      setLoading(false);
    }
  };
  ...
  const cancelar = async () => {
    ...
    try {
      ...
      alert(e?.response?.data?.mensaje || "No se pudo cancelar");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const programarSiguienteMedianoche = () => {
      const now = new Date();
      ...
      return setTimeout(async () => {
        await resetDia();
        midnightTimerRef.current = programarSiguienteMedianoche();
      }, delay);
    };
  ...
  return (
    <div className="pl-wrap" aria-busy={loading ? "true" : "false"}>
      {/* Banner de nueva jornada */}
      {showBanner && (
        <div className="pl-banner">
          Nueva jornada iniciada — la cola fue reiniciada
        </div>
      )}
      ...
      <header className="pl-header">
        <div className="pl-title">
          ...
          <p className="pl-sub">Panel de llamado y control de cola</p>
        </div>

        <div className="pl-controls">
          <div className="pl-vent-group">
            {["A1", "A2", "A3", "B1", "B2", "B3"].map((v) => (
              <button
                key={v}
                type="button"
                className={`pl-vent-btn ${ventanilla === v ? "is-active" : ""}`}
                onClick={() => setVentanilla(v)}
                disabled={loading}
              >
                <FaWindowMaximize /> {v}
              </button>
      ...
      {/* ====== Layout: Izquierda (En atención) | Derecha (Siguiente + Cola) ====== */}
      <div className="pl-grid">
        {/* En atención */}
        <section className="pl-card">
          <div className="pl-card-head">
            <h2>En atención</h2>
            {actual ? <TipoBadge tipo={actual.tipo} /> : null}
          </div>
            ...
            <div className="pl-actual">
              ...
              <div className="pl-row">
                ...
                <div className="pl-kv">
                  ...
                  <span className="v">{actual?.ventanilla ?? ventanilla}</span>
                </div>
              </div>
            </div>
          ) : (
          ...
          <div className="pl-actions">
            ...
            <button
              ...
              disabled={loading || !actual || !actual.id_ticket}
              title="Repetir llamado (R)"
            >
              Repetir <kbd>R</kbd>
            </button>
            <button
        ...
        {/* Derecha: Siguiente en cola + Cola completa con scroll */}
            ...
            <div className="pl-table-wrap">
              <table className="pl-table">
                ...
                <tbody>
                  {cola.length === 0 && (
                    <tr>
                      <td colSpan={5} className="pl-empty-row">
                        Vacío
                      </td>
                    </tr>
                  )}
                  {cola.map((r, i) => {
                    ...
                    return (
                      <tr key={r.id_ticket ?? `${getNo(r)}-${i}`}>
                        ...
                        <td>
                          <span
                            className={`pl-badge sm ${
                              isPref ? "is-pref" : "is-norm"
                            }`}
                          >
                            {isPref ? <FaWheelchair /> : <FaUserClock />}
                            <span>{isPref ? "PREFERENCIAL" : "NORMAL"}</span>
