...
export default function Bitacora() {
  ...
  const generarPDF = () => {
    ...
    const filas = bitacoras.map((b) => [
      ...
      b.usuario,
      // Usa nombre_objeto y, si por alguna razón no viene, cae a id_objeto
      b.nombre_objeto || `ID ${b.id_objeto}`,
      b.accion,
      b.descripcion,
    ]);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Texto a la izquierda → Página X de Y
      doc.text(
  ...
  const filtrar = (r) => {
    ...
    return (
      ...
      (!objeto ||
        (r.nombre_objeto || "")
          .toLowerCase()
          .includes(objeto.toLowerCase())) &&
      (!accion ||
        r.accion?.toLowerCase().includes(accion.toLowerCase())) &&
    ...
    <div className="bitacora-page">
      ...
      <div className="bitacora-topbar">
        ...
        <div className="topbar-actions">
          <Link to="/" className="btn btn-topbar-outline">
      ...
      <div className="bitacora-card">
        <div className="bitacora-filtros">
          ...
          <input
            ...
            onChange={(e) =>
              setFiltros({ ...filtros, objeto: e.target.value })
            }
          />
          <input
            ...
            <tbody>
              {bitacoras.filter(filtrar).map((b) => (
                <tr key={b.id_bitacora}>
                  ...
                  <td>{b.usuario}</td>
                  <td>{b.nombre_objeto || `ID ${b.id_objeto}`}</td>
                  <td>
                    <span
                      className={`action-tag action-${b.accion
                        ?.toUpperCase()
                        .trim()}`}
                    >
                      {b.accion}
