...
export default function HistorialKardex() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // Guardamos el objeto o el mensaje para mostrarlo abajo
      setError(e.response?.data || e.message || "Error al cargar historial");
    } finally {
      setLoading(false);
    }
  };

  // Oculta el error genérico de ruta no encontrada (404 de backend)
        ...
        {loading ? (
          ...
          <table className="inventario-table">
            ...
            <tbody>
              {items.map((it) => (
                <tr
                  key={it.id_historial_kardex || it.id_kardex || it.id}
                >
